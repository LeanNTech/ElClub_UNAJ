	...
	public class Program
{
	...
	{			
		...
		while (repite)
		{
			...
			foreach (Entrenador entrenador in club.listaEntrenadores)
			{
				if (dni == entrenador.Dni)
				{
					club.EliminarEntrenador(entrenador);
					Console.WriteLine("El/la entrenador/a {0} {1} de DNI[{2}] fue dado de baja satisfactoriamente!", entrenador.Nombre, entrenador.Apellido, entrenador.Dni);
					encontrado = true;
					break;
				}					
			}
		...
		do
        {
			...
			if (depo != null)
			{
				try
				{
					...
					{
						Console.WriteLine(" ");
						throw new SinCuposException("No existen cupos disponibles para la categoría adecuada, intente nuevamente con otro deporte.");
					}
				}
				...
				while (repite)
				{
					...
					if (op2 == 1)
					{
						...
						Socio nSocio = new Socio(nombre, apellido, dni, deporte, cate, descuento);
					...
					if (op2 == 2)
					{
						Ninio nNinio = new Ninio(nombre, apellido, dni, deporte, cate);
    ...
    public static void BajaNinioSocio(Club c)
    {
		...
		while (repite)
		{
            ...
            {
                Console.WriteLine($" Nombre: {n.Nombre} Apellido: {n.Apellido} DNI: {n.Dni}, Deporte: {n.Deporte}, Categoría: {n.Cat_}");
            }
        ...
        foreach (Deporte dx in c.Deportes)
        {
            ...
            else
            {
                ...
                foreach (Ninio n in c.Ninios)
                {
                    ...
                    {
                        Console.WriteLine("DNI: [{0}] Nombre y apellido: {1} {2}", n.Dni, n.Nombre, n.Apellido);
                    }
                }                    
				Console.WriteLine("----------------------");
            }
    ...
    public static void ListadoPorDeporteYCategoria(Club c)
    {
        ...
        foreach (Deporte dd in c.Deportes)
        {
            ...
            foreach (Ninio nn in c.Ninios)
            {
                ...
                {
                    Console.WriteLine("DNI: [{0}] Nombre y apellido: {1} {2}", nn.Dni, nn.Nombre,
                    nn.Apellido);
                }
            }                
			Console.WriteLine("----------------");
    ...
    public static void ListadoTotal(Club c)
	{
        ...
        {
            Console.WriteLine("Nombre y apellido: {0} {1}", ninio.Nombre, ninio.Apellido);
            Console.WriteLine("DNI: {0}", ninio.Dni);
            Console.WriteLine("Deporte: {0}", ninio.Deporte);
            Console.WriteLine("Categoría: {0}", ninio.Cat_);
            Console.WriteLine("---------------");
        }
	...
	public static void ListadoDeudores(Club c)
	{
		...
		{
			fechaPago = (DateTime)nx.UltimoMesPago;
			int v = DateTime.Compare(fechaPago, fechaActual);
			if (v < 0)
			{
				Console.WriteLine("[DNI:{0}] Nombre: {1} {2}.", nx.Dni, nx.Apellido, nx.Nombre);
			}
			...
			try
			{
				...
				if (!deporteExiste)
				{
					...
					if (entrenadorExiste)
					{
						...
						Deporte nuevoDeporte = new Deporte(nombreDepo, categoria, cupoCat, nombreEntrenador, apellidoEntrenador, dniEntrenador, diasYHorarios, costoCuota, descuento);
